...
//  In this class we will need to initialise players, allow player to place disc in grid, run game,
...
public class Game implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5753855184842988924L;
	private Grid grid;
    private User[] users;
    ...
    //Main class method Game, which is creating grid and initialising 2 players.
    public Game() {
        users = new User[2];
    ...
    //Method run is responsible for running few other methods like: initialising players, starting a grid,
    //placing disc in grid and adding new move to total moves.
    public void run() throws IOException, ClassNotFoundException {

        //Initialise of the game
        initPlayers();
        grid.startGrid();
        grid.displayGrid();
    ...
    private void initPlayers() {
        ...
        while (true) {
            ...
            //Let's validate first if players entered both names.If both are entered let's start with selection of colours.
            if (users[0].getName().length() > 0 && users[1].getName().length() > 0) {
            	
            	//Player 1 selects disc's colour.
            	int player_one = JOptionPane.showOptionDialog(null,users[0].getName() + ", which colour do you want to play with?","Choose colour", 
            					JOptionPane.PLAIN_MESSAGE,JOptionPane.PLAIN_MESSAGE,null, colours,colours[0]);
                ...
                if(player_one == 0){
                    ...
                    users[0].setDisplayColour(red);
                    users[1].setDisplayColour(yellow);
                    print(users[1].getName() + ", you will play with colour " + colours[0] + ".");
                }
                break;
            }
            //If player 2 didn't enter his name.
            else if(users[0].getName().length() > 0 && users[1].getName().length() == 0){
            	String error = "Player 2, you need to enter your name!";
            	JOptionPane.showMessageDialog(null, error, "Error!", JOptionPane.ERROR_MESSAGE, null);
            	continue;
            }
            //If player 1 didn't enter his name.
            else if(users[0].getName().length() == 0 && users[1].getName().length() > 0){
            	String error = "Player 1, you need to enter your name!";
            	JOptionPane.showMessageDialog(null, error, "Error!", JOptionPane.ERROR_MESSAGE, null);
            	continue;
            }
            //If both players didn't enter their names.
            else{
            	String error = "You need to enter both player names!";
            	JOptionPane.showMessageDialog(null, error, "Error!", JOptionPane.ERROR_MESSAGE, null);
            	continue;
            }
        }//end of while loop
    ...
    //Method to save game. Save game status will be saved in text file "savegame.txt".
